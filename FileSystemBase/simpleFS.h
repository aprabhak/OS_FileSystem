...
extern size_t fblockwrite(const void * ptr, size_t size, size_t count, FILE * stream);

extern char ** parsepath(char * path, int size, int * depth);

extern uint32_t contains(struct inode * dir, char * dirName);

extern int firstfreebit(unsigned char * bitmap);
